...
if selected_iso3 in ("USA", "CHN"):
    ...
    st.markdown(f"### Who is most diplomatically aligned with {selected_name}? ({latest_year})")
    ...
    with col_most:
        st.markdown("**Most diplomatically aligned**")
        top = country_dyads.nlargest(10, "structural_alignment")[
            ["partner_name", "structural_alignment"]].reset_index(drop=True)
        top.columns = ["Country", "Diplomatic Alignment"]
        top.index = top.index + 1
        st.dataframe(top.style.format({"Diplomatic Alignment": "{:.3f}"}), use_container_width=True)
    with col_least:
        st.markdown("**Least diplomatically aligned**")
        bottom = country_dyads.nsmallest(10, "structural_alignment")[
            ["partner_name", "structural_alignment"]].reset_index(drop=True)
        bottom.columns = ["Country", "Diplomatic Alignment"]
        bottom.index = bottom.index + 1
        st.dataframe(bottom.style.format({"Diplomatic Alignment": "{:.3f}"}), use_container_width=True)
    st.stop()
...
# ── Change over time ──
st.markdown("### How has diplomatic alignment changed?")
...
with col_a:
    ...
    st.metric(
        f"Change in diplomatic alignment with US ({int(earliest['year'])}→{latest_year})",
...
with col_b:
    ...
    st.metric(
        f"Change in diplomatic alignment with China ({int(earliest['year'])}→{latest_year})",
        f"{latest['alignment_with_China']:.2f}",
        f"{delta_cn:+.3f}",
    )

# ── Most and least aligned ──
st.markdown(f"### Who is {selected_name} most diplomatically aligned with? ({latest_year})")
